...
//   ./build/eposlib/api_demo $CFG/master.dcf vcan0 2
//
...
main(int argc, char ** argv)
{
  ...
  // ---- power and thermal, in physical units ----
  // The first thing worth seeing on a new drive: is the battery what it
  // should be, and how far is the power stage from its thermal limit.
  //
  // RefreshAll reads them concurrently. With one drive they still queue on
  // its SDO channel; with several, reads to different drives overlap.
  auto & supply = motor.GetSupplyVoltage();
  auto & temperature = motor.GetPowerStageTemperature();
  auto & limit = motor.GetPowerStageTemperatureLimit();
  auto & current = motor.GetMotorCurrentAveraged();
  if (auto ec = epos4::signals::RefreshAll(supply, temperature, limit, current)) {
    printf("power/thermal read failed: %s\n", ec.message().c_str());
  }
  if (epos4::signals::IsAllGood(supply, temperature, limit, current)) {
    printf("supply   : %.1f V\n", supply.GetValue().value());
    printf(
      "power stage: %.1f degC (limit %.1f degC)\n",
      temperature.GetValue().value(), limit.GetValue().value());
  ...
  if (ec) {
    ...
    for (int i = 0; i < 60; ++i) {
      if (motor.IsTargetReached().Refresh().GetValue()) {break;}
      printf(
        "  moving... position = %d, current = %.3f A\n",
        motor.GetPosition().Refresh().GetValue(),
