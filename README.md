...
```cpp
epos4::CanBus bus{{"can0", dcf}};   // interface, master DCF
...
> **Status: 0.1.0 plus unreleased work, pre-release.** The library, the
> `ros2_control` plugin and the bring-up have been verified end to end against
> the included simulator: a `joint_trajectory_controller` trajectory reaches
> the axis over PDO, a dead drive is detected in ~65 ms, and a dead master in
> 300 ms. **Nothing has run against a physical EPOS4 yet.** See
...
- [Under ros2_control](#under-ros2_control)
...
epos4_core   pure CiA 402 logic. No Lely, no CAN, no ROS.
             Compiles with plain g++. Tested without a bus.
                    ^
...
| **`epos4_ros2_control`** | `hardware_interface::SystemInterface` plugin: one CAN bus, one EPOS4 per joint, Cyclic Synchronous Position |
| **`epos4_bringup`** | CANopen network description (`bus.yml`), device EDS files, generated DCFs, `ros2_control` xacro macros, controllers and launch |
| **`epos4_interfaces`** | ROS messages and services — *empty, placeholder* |
| **`robot_units`** | [nholthaus/units](https://github.com/nholthaus/units) v2.3.5, vendored, for the whole robot |
...
`epos4_ros2_control/Epos4System` drives every joint in **Cyclic Synchronous
Position**: the trajectory is generated by the controller and the drive
interpolates between setpoints, which is the model
`joint_trajectory_controller` assumes. Profile Position generates the ramp
inside the drive and does not fit it.

```
command interface:   position                    rad at the joint
state interfaces:    position, velocity, effort  rad, rad/s, Nm
```

`read()` and `write()` never touch the bus. Feedback arrives by TPDO into
lock-free atomics on the CANopen thread, and setpoints are staged into
atomics that the same thread publishes on the next SYNC.

### Describing the axes

`epos4_bringup/config/epos4.ros2_control.xacro` provides two macros, so the
plugin knows nothing about any particular arm:

```xml
<xacro:include filename="$(find epos4_bringup)/config/epos4.ros2_control.xacro"/>

<xacro:epos4_system name="arm" master_dcf="${dcf}" can_interface="can0">
  <joints>
    <xacro:epos4_joint name="shoulder" node_id="2"
                       quadcounts_per_revolution="2000" gear_ratio="0.01"/>
  </joints>
</xacro:epos4_system>
```

`quadcounts_per_revolution` is **four times** the encoder's pulses per
revolution (500 CPR → 2000). `gear_ratio` is output over motor (1:100 →
0.01). The node-ID must match `bus.yml`, and every joint must also exist as a
URDF joint; `epos4_arm.urdf.xacro` is a minimal one-axis description for
bring-up.

### Launching

```bash
ros2 launch epos4_bringup arm.launch.py                        # epos4_sim on vcan0
ros2 launch epos4_bringup arm.launch.py can_interface:=can0    # hardware
```

Keep `update_rate` in `controllers.yaml` equal to the SYNC rate of
`bus.yml` (100 Hz for `sync_period: 10000`), and «Interpolation time
period» (0x60C2) in step with both.
...
ros2 topic pub -w 1 --once /arm_controller/joint_trajectory \
  ...
  "{joint_names: [shoulder], points: [{positions: [0.5], time_from_start: {sec: 3}}]}"
candump vcan0 | grep ' 202 '           # a new Target position on every SYNC
```

`-w 1` waits for the subscriber. Without it `--once` can publish before DDS
discovery finishes, and the axis appears not to respond.
...
```cpp
epos4::CanBus bus{{"can0", "/path/to/master.dcf"}};   // interface, master DCF
...
master:
  node_id: 1
  baudrate: 1000
  heartbeat_producer: 100   # ms; drives time out after 3x this
...
node_2:
  ...
  heartbeat_consumer: true  # watch the master; see "When something dies"
...
There are two networks: `epos4_network` for hardware **and for `epos4_sim`**,
and `sim_network` for the `canopen_fake_slaves` mock only. They are separate
because the master verifies slave identity, and the mock reports vendor
`0x555` where a real maxon drive (and `epos4_sim`) reports `0xFB`. That
rejection is correct behaviour — it is what stops a miswired axis from being
driven — so the mock gets its own DCF rather than weakening it.

Pointing `epos4_sim` at `sim_network` fails quietly: the boot aborts with
`es='D'` before the PDO mapping is downloaded, and the master then decodes the
simulator's default-mapped frames as if they were its own.
...
Two failures matter on a moving arm, and each is caught from the other side.

**A drive goes silent** (cable, power, node crash). Its cached feedback stops
changing but keeps reading back as if nothing happened, so every cycle
`read()` checks `IsCyclicHealthy()`: PDOs fresher than 50 ms and the drive in
«Operation enabled». When that fails, `read()` returns `ERROR` and logs once,
from memory only - never an SDO read against a node that cannot answer.
`on_error` then stops the cyclic path and commands every axis to disable. The
bus is kept up on purpose, so the master keeps sending «Disable voltage» on
every SYNC and a drive that reconnects is told to drop power, not to resume.

**The master dies** (the process running `ros2_control`). Nothing on the
master side can react to that, so the drives watch for it themselves:
`heartbeat_consumer: true` in `bus.yml` makes `dcfgen` configure «Consumer
heartbeat time» (0x1016) in each drive. With the master producing every
100 ms, a drive notices within 300 ms, raises «CAN heartbeat error» (0x8130),
stops according to «Abort connection option code» (0x6007), and drops to NMT
pre-operational.

Both are verified against `epos4_sim`: killing the simulator is detected in
~65 ms, and `kill -9` on `ros2_control_node` produces EMCY 0x8130 300 ms after
the last heartbeat.
...
- **Nothing has run against a physical EPOS4.** Everything is verified against
  `epos4_sim`.
- Cyclic Synchronous Position is exercised end to end against the simulator.
  Homing, CSV and CST are implemented but unexercised.
